...
extern struct macosx_dyld_thread_status macosx_dyld_status;
#endif
...
* Exposed helper
***************************************************************************/
struct current_debug_context_cleanup
{
...
static void
do_restore_current_debug_context_cleanup (void *arg)
{
	struct current_debug_context_cleanup *old = arg;
	struct frame_info* selected_frame = get_selected_frame (NULL);
	// thread
	if (!ptid_equal (old->inferior_ptid, inferior_ptid))
		switch_to_thread (old->inferior_ptid);
	// frame
	if (selected_frame && frame_relative_level(selected_frame) != old->frame_num)
	{
		struct frame_info *current_frame = get_current_frame();
		while (current_frame)
		{
			if (frame_relative_level(current_frame) == old->frame_num)
			{
				select_frame(current_frame);
				break;
			}
			current_frame = get_prev_frame(current_frame);
		}
	}
	xfree (old);
}

// remember/restore current selected thread/frame
struct cleanup *
make_cleanup_restore_current_debug_context ()
{
	struct current_debug_context_cleanup *old
				= xmalloc (sizeof (struct current_debug_context_cleanup));
	struct frame_info* selected_frame = get_selected_frame (NULL);
	old->inferior_ptid = inferior_ptid;
	if (selected_frame)
		old->frame_num = frame_relative_level (selected_frame);
	else
		old->frame_num = 0;

	return make_cleanup (do_restore_current_debug_context_cleanup, old);
}

// Read data from target's address space
...
{
	address_t rsp = read_register_pid(g_rsp_regnum, info->ptid);
...
static int mmap_core_file(const char* fname)
{
	...
	{
		struct mach_header_64* mach_header = (struct mach_header_64*) mpStartAddr;
		// Traverse all load commands
		unsigned int i;
		...
		for (i = 0; i < mach_header->ncmds; i++)
		{
			if (load_cmd->cmd == LC_SEGMENT_64)
			{
				...
				struct ca_segment* a_seg = add_one_segment(seg_cmd->vmaddr,
														seg_cmd->vmsize,
														seg_cmd->initprot & VM_PROT_READ,
														seg_cmd->initprot & VM_PROT_WRITE,
														seg_cmd->initprot & VM_PROT_EXECUTE);
				a_seg->m_faddr = mpStartAddr + seg_cmd->fileoff;
				a_seg->m_fsize = seg_cmd->filesize;
			}
			else if (load_cmd->cmd == LC_THREAD)
			{
			}
			else
				printf("Unknown load command[%d]: %d\n", i, load_cmd->cmd);
			load_cmd = (struct load_command*) ((char*)load_cmd + load_cmd->cmdsize);
		}
		// sanity check
		if ((char*)load_cmd != (char*)(mach_header + 1) + mach_header->sizeofcmds)
		{
			printf("Actual load cmd size %ld is not consistent with mach_header %d\n",
					(char*)load_cmd - (char*)(mach_header + 1), mach_header->sizeofcmds);
		}
	}
#endif
...
static CA_BOOL find_memory_regions(CA_BOOL create)
{
#ifdef linux
	return linux_nat_find_memory_regions(create);
#elif defined(__MACH__)
	// !FIXME!
	return CA_FALSE;
#endif
...
static int build_segments()
{
	...
	g_ptr_bit = gdbarch_ptr_bit(current_gdbarch);
...
#elif defined(__MACH__)
		// !FIXME!
		printf_filtered(_("Not implemented for live process yet\n"));
		return CA_FALSE;
#endif
	}
	else if (core_bfd)
	{
		...
		else if (core_bfd->xvec->flavour == bfd_target_mach_o_flavour)
		{
			if (mmap_core_file(bfd_get_filename(core_bfd)) == CA_FALSE)
			{
				error (_("Failed to mmap core file %s"), bfd_get_filename(core_bfd));
...
#elif defined(__MACH__)
	{
		...
		for (i = 0; i < s->nents; i++)
		{
			...
			struct dyld_objfile_entry *e = &s->entries[i];
			struct objfile* objfile;
			struct obj_section *os;
			const char* text_name = "__TEXT.__text";
			const char* data_name = "__DATA.__";
			...
			{
				enum storage_type type = ENUM_MODULE_DATA;
				CA_BOOL setup_seg = CA_FALSE;
				segment = get_segment(os->addr, 1);
				if (segment)
				{
					if (strstr (os->the_bfd_section->name, text_name) == 0)
						type = ENUM_MODULE_TEXT;
					else if (strcmp (os->the_bfd_section->name, data_name) == 0)
						type = ENUM_MODULE_DATA;
					if (e->image_name_valid)
						name = e->image_name;
					else if (e->dyld_name_valid)
						name = e->dyld_name;

					if (segment->m_type == ENUM_UNKNOWN)
						setup_seg = CA_TRUE;
					else if ((segment->m_type == ENUM_MODULE_TEXT || segment->m_type == ENUM_MODULE_DATA)
						&& (segment->m_vaddr != os->addr || segment->m_vsize != os->endaddr - os->addr)
						&& (segment->m_type != type || strcmp(segment->m_module_name, name)) )
					{
						segment = add_one_segment(os->addr, os->endaddr - os->addr, segment->m_read, segment->m_write, segment->m_exec);
						if (segment)
						{
							setup_seg = CA_TRUE;
				...
				{
					segment->m_type = type;
					segment->m_module_name = strdup(name);
				}
			}
		}
	}
	...
	/* thread stacks */
	{
		...
		g_rsp_regnum = gdbarch_sp_regnum (current_gdbarch);
...
CA_BOOL update_memory_segments_and_heaps()
{
	...
	if (g_segments && g_segment_count)
	{
		...
		if (g_debug_core || find_memory_regions(CA_FALSE))
			return CA_TRUE;
	...
	old_chain = make_cleanup_restore_current_debug_context();
		...
		if (bufsz >= numregs)
		{
			...
			for (i=0; i<numregs; i++)
			{
				...
				if (regs[i].reg_width <= ptr_sz)
					regs[i].value = (address_t) read_register_pid(i, info->ptid);
				else
					regs[i].value = 0;
			}
			return numregs;
						...
						struct CA_LIST* refs)
{
	...
	const struct thread_info *info = (const struct thread_info*) segment->m_thread.context;
	{
		...
		for (i=0; i<numregs; i++)
		{
			...
			{
				ULONGEST val = read_register_pid(i, info->ptid);
...
CA_BOOL is_heap_object_with_vptr(const struct object_reference* ref, char* name_buf, size_t buf_sz)
{
	...
	if (target_read_memory(addr, (void*)&val, ptr_sz) == 0 && val)
	{
		...
		if (segment && (segment->m_type == ENUM_MODULE_DATA || segment->m_type == ENUM_MODULE_TEXT) )
		{
			...
			if (build_address_symbolic (vptr, 1 /*do_demangle*/,
				...
				size_t offset, int lea, char* namebuf, size_t bufsz, int* is_vptr)
{
	*namebuf = '\0';
	...
	if (TYPE_CODE(basetype) == TYPE_CODE_STRUCT || TYPE_CODE(basetype) == TYPE_CODE_UNION)
	{
		...
		vptr_fieldno = TYPE_VPTR_FIELDNO (basetype); // -1 if the struct doesn't have a vptr
		...
		for (i = 0; i < num_fields; i++)
		{
			...
			if (TYPE_FIELD_STATIC(basetype, i))	// static member
				continue;
...
print_type_name(struct type *type, const char* field_name, const char* prefix, const char* postfix)
{
	...
	else if (TYPE_CODE (type) == TYPE_CODE_ARRAY)
	{
		...
		{
			printf_filtered(_("[%d]"), TYPE_HIGH_BOUND (range_type)+1);
		}
		else
			printf_filtered(_("[]\")"));
		if (field_name)
...
print_struct_field(const struct object_reference* ref, struct type *type, size_t offset)
{
	...
	else if (TYPE_CODE(type) == TYPE_CODE_STRUCT || TYPE_CODE(type) == TYPE_CODE_UNION)
	{
		...
		for (i = 0; i < num_fields; i++)
		{
			...
			field_size = field_type->length;
			if (TYPE_FIELD_STATIC(type, i))	// static member
				continue;
			if ( i+1 < num_fields
				&& field_size == 1
...
static struct minimal_symbol* get_global_minimal_sym(const struct object_reference* ref, address_t* sym_addr, size_t* sym_sz)
{
	...
	msymbol = lookup_minimal_symbol_by_pc_section (ref->vaddr, section);
	if (msymbol)
	{
...
struct symbol* get_global_sym(const struct object_reference* ref, address_t* sym_addr, size_t* sym_sz)
{
	...
	symbol = find_pc_sect_function (ref->vaddr, section);

	if (symbol)
	{
		struct type* type = symbol->type;
		CORE_ADDR name_location = BLOCK_LOWEST_PC (SYMBOL_BLOCK_VALUE (symbol));
		unsigned int offset = ref->vaddr - name_location;
		add_addr_type (ref->vaddr - offset, type, symbol);
		if (sym_addr && sym_sz)
		{
...
get_stack_sym(const struct object_reference* ref, address_t* sym_addr, size_t* sym_sz)
{
	...
	if (ref->where.stack.frame >= 0)
	{
		...
		if (pc && (block = get_frame_block (frame, 0) ) )
		{
...
struct type* get_heap_object_type(const struct object_reference* ref)
{
	...
	if (ref->storage_type == ENUM_HEAP && ref->where.heap.inuse)
	{
		...
		else if (is_heap_object_with_vptr(ref, obj_name, NAME_BUF_SZ))
		{
			...
			struct symbol *sym = lookup_symbol (obj_name, 0, STRUCT_DOMAIN, 0, NULL);
			if (sym)
			{
...
void print_global_ref (const struct object_reference* ref)
{
	...
	{
		printf_filtered(_(" %s 0x%lx"), SYMBOL_PRINT_NAME (msym), sym_addr);
		if (ref->target_index >= 0)
			printf_filtered (_(" @0x%lx"), ref->vaddr);
	}
	else
...
static struct type * get_real_type_from_exp(char* exp)
{
    ...
    if (!type)
    	type = parse_and_eval_type(exp, strlen(exp));
	return type;
}
		...
		if (TYPE_CODE(type) == TYPE_CODE_CLASS && TYPE_NAME (type))
		{
			...
			if (msym)
			{
				...
				if (msym->size)
					...
					vtable->high = vtable->low + 32;	// !FIXME! why msym->size==0?
...
static CA_BOOL ca_get_frame_register(struct frame_info *frame, int regnum, size_t* valp)
{
	...
	frame_register_unwind (frame, regnum, &optimized,
			&lval, &addr, &realnum, (gdb_byte *)valp);

	if (optimized != opt_okay)
		return CA_FALSE;
	else
		return CA_TRUE;
}
...
{
	int ptr_bit = g_ptr_bit;
	//struct ui_out *uiout = uiout;	// later version of gdb renamed uiout to current_uiout
		...
		if (arg_list != 0)
		{
			...
			ALL_BLOCK_SYMBOLS (b, iter, sym)
			{
				...
				if (*SYMBOL_LINKAGE_NAME (sym))
				{
					...
					nsym = lookup_symbol (SYMBOL_LINKAGE_NAME (sym), b, VAR_DOMAIN, 0, NULL);

					if (SYMBOL_CLASS (nsym) == LOC_REGISTER
							&& SYMBOL_CLASS (nsym) != LOC_ARG)
					{
						;
					}
					else
				...
				// find the corresponding register used to pass this parameter
				{
					...
					if (reg_name)
					{
						...
						fputs_filtered (reg_name, stb->stream);
						fputs_filtered (")", stb->stream);
					...
					CA_BOOL verbose)
{
	...
	for (i = 0; i < numregs; i++)
	{
			...
			/*&& gdbarch_register_reggroup_p (gdbarch, i, all_reggroup)*/)
		{
			...
			enum lval_type lval;
			CORE_ADDR addr;
			frame_register_unwind (selected_frame, i, &optimized, &lval,
									&addr, &realnum, NULL);
			if (optimized == opt_okay && lval == lval_memory)
			{
...
void calc_heap_usage(char *exp)
{
	...
	if (val && value_type(val))
	{
		...
		val_type = *deprecated_value_lval_hack (val);
		...
		if (var_len >= ptr_sz)
		{
			...
			{
				ref.vaddr = *deprecated_value_address_hack (val);
				fill_ref_location(&ref);
			}
...
{
	return parse_and_eval_address((char*)expr);
}
