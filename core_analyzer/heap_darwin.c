								...
								struct ca_region_stats* stats)
{
	...
	//if (verbose)
	CA_PRINT("\tsmall region "PRINT_FORMAT_POINTER" - "PRINT_FORMAT_POINTER"\n",
			ptr, region_end);
	...
	while (ptr < region_end)
	{
		...
		if (!(msize_and_free & SMALL_IS_FREE))
		{
			...
			{
				stats->num_inuse++;
				stats->inuse_bytes += SMALL_BYTES_FOR_MSIZE(msize);
			}
			// move to next block
		...
		else
		{
			...
			{
				stats->num_free++;
				stats->free_bytes += SMALL_BYTES_FOR_MSIZE(msize);
			}
			// move to next block
...
static CA_BOOL
tiny_region_walk(szone_t* szonep, region_t region, CA_BOOL verbose, struct ca_region_stats* stats)
{
	CA_BOOL rc = CA_TRUE;
	...
	//if (verbose)
	CA_PRINT("\ttiny region "PRINT_FORMAT_POINTER" - "PRINT_FORMAT_POINTER"\n",
			ptr, region_end);
	...
	while (ptr < region_end)
	{
		...
		if (!is_free)
		{
			...
			{
				stats->num_inuse++;
				stats->inuse_bytes += TINY_BYTES_FOR_MSIZE(msize);
			}
			// move to next block
		...
		else
		{
			...
			{
				stats->num_free++;
				stats->free_bytes += TINY_BYTES_FOR_MSIZE(msize);
			}
			// move to next block
...
static CA_BOOL
szone_walk(szone_t* szonep, CA_BOOL verbose, struct ca_region_stats* statsp)
{
	...
	if (num_regions > 0)
	{
		...
		for (index = 0; index < num_regions; index++)
		{
			...
			if (region && region != HASHRING_REGION_DEALLOCATED)
			{
				...
				tiny_region_walk(szonep, region, verbose, &tiny_stats);
	...
	if (num_regions > 0)
	{
		...
		for (index = 0; index < num_regions; index++)
		{
			...
			if (region && region != HASHRING_REGION_DEALLOCATED)
			{
				...
				small_region_walk(szonep, region, verbose, &stats);
				...
				&& entry.address)
			{
				...
				if (verbose)
				{
					CA_PRINT("\tlarge block "PRINT_FORMAT_POINTER" - "PRINT_FORMAT_POINTER" size=%ld in-use\n",
							entry.address, entry.address + entry.size, entry.size);
				}
			}
		}
		CA_PRINT("\t%d in-use large regions\n", count);
			...
			if (entryp->address)
			{
				...
				if (verbose)
				{
					CA_PRINT("\tlarge block "PRINT_FORMAT_POINTER" - "PRINT_FORMAT_POINTER" size=%ld cached\n",
							entryp->address, entryp->address + entryp->size, entryp->size);
				}
			}
		}
		CA_PRINT("\t%d cached large regions\n", count);
	}
	if (statsp)
	{
...
static CA_BOOL build_szones(void)
{
	...
	malloc_zone_vaddr = get_var_addr_by_name("malloc_zones");
	malloc_num_zone_vaddr = get_var_addr_by_name("malloc_num_zones");
...
static void build_tiny_region_blocks(struct ca_region* regionp)
{
	...
	if (count == 0)
		return;
	regionp->blocks = (address_t*) malloc(sizeof(address_t) * count);
...
static void build_small_region_blocks(struct ca_region* regionp)
{
	...
	if (count == 0)
		return;
	regionp->blocks = (address_t*) malloc(sizeof(address_t) * (count + 1));
	// Second pass would populate the array with block addresses with inuse/free bit
	ptr = first_block;
	count = 0;
	while (ptr < region_end)
